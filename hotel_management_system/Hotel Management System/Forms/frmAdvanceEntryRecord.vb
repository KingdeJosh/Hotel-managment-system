...
Public Class frmAdvanceEntryRecord
    ...
    Public Sub GetData()
        Try
            ...
            con.Open()
            cmd = New SqlCommand("select RTRIM(AdvanceEntry.ID) as [ID], Convert(DateTime,workingdate,131) as [Entry Date],RTRIM(EmployeeRegistration.ID) as [Emp ID],RTRIM(EmployeeRegistration.EmployeeID) as [Employee ID],RTRIM(EmployeeName) as [Employee Name],RTRIM(Amount) as [Advance] from Advanceentry,employeeRegistration where EmployeeRegistration.ID=AdvanceEntry.EmployeeID and Amount > 0 order by workingdate", con)
        ...
        Try
            If dgw.Rows.Count > 0 Then
                If lblSet.Text = "Advance Entry" Then
                    ...
                    frmAdvanceEntry.txtEmpID.Text = dr.Cells(2).Value.ToString()
                    frmAdvanceEntry.txtEmployeeID.Text = dr.Cells(3).Value.ToString()
                    frmAdvanceEntry.txtEmployeeName.Text = dr.Cells(4).Value.ToString()
    ...
    Private Sub txtEmployeeName_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtEmployeeName.TextChanged
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("select RTRIM(AdvanceEntry.ID) as [ID], Convert(DateTime,workingdate,131) as [Entry Date],RTRIM(EmployeeRegistration.ID) as [Emp ID],RTRIM(EmployeeRegistration.EmployeeID) as [Employee ID],RTRIM(EmployeeName) as [Employee Name],RTRIM(Amount) as [Advance] from Advanceentry,employeeRegistration where EmployeeRegistration.ID=AdvanceEntry.EmployeeID and Amount > 0 and EmployeeName like '" & txtEmployeeName.Text & "%' order by workingdate", con)
            Dim myDA As SqlDataAdapter = New SqlDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "AdvanceEntry")
            myDA.Fill(myDataSet, "EmployeeRegistration")
