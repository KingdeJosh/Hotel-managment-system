...
Public Class frmAdvanceEntry
    ...
    Sub GetData()
        Try
            ...
            con.Open()
            Dim sql As String = "SELECT RTRIM(EmployeeRegistration.ID),RTRIM(EmployeeRegistration.EmployeeID),RTRIM(EmployeeName),sum(Amount)-sum(Deduction) FROM EmployeeRegistration left join AdvanceEntry on EmployeeRegistration.ID=AdvanceEntry.EmployeeID group by employeename,employeeregistration.employeeid,EmployeeRegistration.ID order by EmployeeName"
    ...
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        If Len(Trim(txtEmployeeID.Text)) = 0 Then
            MessageBox.Show("Please retrieve employee info", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtEmployeeID.Focus()
            Exit Sub
        End If
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into advanceentry(ID,workingdate,employeeid,amount,deduction) VALUES (" & txtID.Text & ",@d1," & txtEmpID.Text & "," & txtAmount.Text & ",0)"
    ...
    Private Sub btnUpdate_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update advanceentry set EmployeeID=" & txtEmpID.Text & ",Amount=" & txtAmount.Text & " where id=" & txtID.Text & ""
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.ExecuteReader()
            If con.State = ConnectionState.Open Then
