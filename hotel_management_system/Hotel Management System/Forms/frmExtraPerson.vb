...
Public Class frmExtraPerson
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con.Open()
            Dim ct As String = "select RoomType from ExtraPerson where RoomType=@d1"
            ...
            Dim cb As String = "insert into ExtraPerson(RoomType,Charges) VALUES (@d1," & txtChargesPerDay.Text & ")"
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", cmbRoomType.Text)
            cmd.ExecuteReader()
            con.Close()
            Dim st As String = "added the new ExtraPerson record for '" & cmbRoomType.Text & "'"
    ...
    Private Sub DeleteRecord()
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cq As String = "delete from ExtraPerson where RoomType=@d1"
            cmd = New SqlCommand(cq)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", cmbRoomType.Text)
            RowsAffected = cmd.ExecuteNonQuery()
            If RowsAffected > 0 Then
                Dim st As String = "deleted the ExtraPerson record of '" & cmbRoomType.Text & "'"
                LogFunc(lblUser.Text, st)
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "update ExtraPerson set RoomType=@d1,Charges=" & txtChargesPerDay.Text & " where RoomType=@d2"
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", cmbRoomType.Text)
            ...
            cmd.ExecuteReader()
            con.Close()
            Dim st As String = "updated the ExtraPerson '" & cmbRoomType.Text & "' details"
            LogFunc(lblUser.Text, st)
            MessageBox.Show("Successfully updated", "ExtraPerson Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
            btnUpdate.Enabled = False
            Getdata()
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.[Error])
    ...
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(RoomType), RTRIM(Charges) from ExtraPerson order by RoomType", con)
    ...
    Private Sub frmExtraPerson_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Getdata()
        fillCombo()
    End Sub
