...
Public Class frmAttendance
    ...
    Sub GetData()
        Try
            ...
            con.Open()
            Dim sql As String = "SELECT RTRIM(EmployeeRegistration.ID),RTRIM(EmployeeRegistration.EmployeeID),RTRIM(EmployeeName),RTRIM(BasicWorkingTime) from EmployeeRegistration order by EmployeeName"
    ...
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into employeeAttendance(ID,Workingdate,employeeid,status,intime,outtime,overtime,basicworkingtime) VALUES (" & txtID.Text & ",@d1,@d2,@d3,@d4,@d5,@d6,@d7)"
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", WorkingDate.Value.Date)
            ...
            cmd.Parameters.AddWithValue("@d8", WorkingDate.Text)
            cmd.ExecuteReader()
    ...
    Private Sub auto()
        ...
        con = New SqlConnection(cs)
        con.Open()
        Dim sql As String = ("SELECT MAX(ID) FROM EmployeeAttendance")
    ...
    Private Sub txtEmployee_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtEmployee.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "SELECT RTRIM(EmployeeRegistration.ID),RTRIM(EmployeeRegistration.EmployeeID),RTRIM(EmployeeName),RTRIM(BasicWorkingTime) from EmployeeRegistration where EmployeeName like '" & txtEmployee.Text & "%' order by EmployeeName"
            cmd = New SqlCommand(sql, con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
