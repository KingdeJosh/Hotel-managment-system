...
Public Class frmGuest
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into Guest(ID,GuestID, GuestName, Address, City, ContactNo, IDType, IDNumber, Notes,Gender,Religion,Occupation,Pic1,Pic2,Pic3) VALUES (" & txtID.Text & ",'" & txtGuestID.Text & "',@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8,@d9,@d10,@d11,@d12,@d13)"
    ...
    Private Sub btnUpdate_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update Guest set GuestID='" & txtGuestID.Text & "', GuestName=@d1, Address=@d2, City=@d3, ContactNo=@d4, IDType=@d5, IDNumber=@d6, Notes=@d7,Gender=@d8,Religion=@d9,Occupation=@d10,Pic1=@d11,Pic2=@d12,Pic3=@d13 where ID=" & txtID.Text & ""
            cmd = New SqlCommand(cb)
            cmd.Parameters.AddWithValue("@d1", txtGuestName.Text)
            cmd.Parameters.AddWithValue("@d2", txtGuestAddress.Text)
            cmd.Parameters.AddWithValue("@d3", txtGuestCity.Text)
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb1 As String = "delete from Guest_Docs where GuestID=" & txtID.Text & ""
    ...
    Public Sub DeleteRecord()
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl1 As String = "select Guest.ID from Guest,Reservation where Guest.ID=Reservation.GuestID and Guest.ID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl2 As String = "select Guest.ID from Guest,Reservation_Hall where Guest.ID=Reservation_Hall.GuestID and Guest.ID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl3 As String = "select Guest.ID from Guest,Reservation_Garden where Guest.ID=Reservation_Garden.GuestID and Guest.ID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cq As String = "delete from guest where GuestID='" & txtGuestID.Text & "'"
            cmd = New SqlCommand(cq)
            cmd.Connection = con
            RowsAffected = cmd.ExecuteNonQuery()
            If RowsAffected > 0 Then
                Dim st As String = "deleted the guest having Guest ID '" & txtGuestID.Text & "'"
                LogFunc(lblUser.Text, st)
