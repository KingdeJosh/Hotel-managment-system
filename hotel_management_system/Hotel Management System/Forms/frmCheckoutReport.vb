...
Public Class frmCheckoutReport
    Sub fillRoomNo()
        Try
            ...
            adp.SelectCommand = New SqlCommand("SELECT distinct RTRIM(Room.RoomNo) FROM CheckOut_Room,Guest,Room,CheckIN_Room where Guest.ID=CheckIN_Room.GuestID and Checkout_Room.CheckInID=CheckIn_Room.Cin_ID and Room.RoomNo=CheckIn_Room.RoomNo", CN)
            ds = New DataSet("ds")
            adp.Fill(ds)
            dtable = ds.Tables(0)
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        Try
            ...
            Timer1.Enabled = True
            Dim rpt As New rptCheckOut 'The report you created.
            ...
            MyCommand.CommandText = "SELECT Checkin_Room.Cin_Id,Guest.ID,CheckIN_Room.GuestID, CheckIN_Room.RoomNo, CheckIN_Room.RoomCharges, CheckIN_Room.DateIN, CheckIN_Room.DateOUT, CheckIN_Room.NoOfDays, CheckIN_Room.NoOfMales,CheckIN_Room.NoOfFemales, CheckIN_Room.NoOfKids, CheckIN_Room.NoOfExtraBed, CheckIN_Room.NoOfExtraPerson, CheckIN_Room.RoomOrderCharges, CheckIN_Room.ExtraPersonCharges, CheckIN_Room.TotalRoomCharges, CheckIN_Room.ExtraBedCharges, CheckIN_Room.OtherCharges, CheckIN_Room.DiscountPer, CheckIN_Room.DiscountAmount, CheckIN_Room.STPer,CheckIN_Room.STAmount, CheckIN_Room.LuxuryTaxPer, CheckIN_Room.LuxuryTaxAmount, CheckIN_Room.SubTotal, CheckIN_Room.GrandTotal, CheckIN_Room.TotalPaid, CheckIN_Room.Balance,CheckIN_Room.Status, CheckIN_Room.TaxPaidRate, CheckIN_Room.Notes, Room.RoomType, Guest.GuestName, Guest.Address, Guest.City, Guest.ContactNo, Guest.Gender, Guest.Occupation, Guest.Religion, Guest.IDType, Guest.IDNumber, Guest.Pic1, Guest.Pic2, Guest.pic3, Guest.Notes AS Expr5,Guest_Docs.Id AS Expr6, Guest_Docs.GuestID AS Expr7, Guest_Docs.Image1, Guest_Docs.Image2, Guest_Docs.Image3 FROM CheckIN_Room INNER JOIN Room ON CheckIN_Room.RoomNo = Room.RoomNo INNER JOIN Guest ON CheckIN_Room.GuestID = Guest.ID INNER JOIN Guest_Docs ON Guest.ID = Guest_Docs.GuestID where DateOut between @d1 and @d2 and Status='Checked Out' order by DateOut"
    ...
    Private Sub Button3_Click(sender As System.Object, e As System.EventArgs) Handles Button3.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("Select RTRIM(CheckOut_Room.ID) as [Bill ID],RTRIM(BillNo) as [Bill No.],Convert(Datetime,BillDate,131) as [Bill Date],RTRIM(CheckIn_Room.Cin_Id) as [Check In ID], RTRIM(Guest.GuestID) as [Guest ID],RTRIM(GuestName) as [Guest Name],RTRIM(Room. RoomNo) as [Room No.], RTRIM(CheckIN_Room.RoomCharges) as [Room Charges], Convert(DateTime,DateIN,103) as [Date IN], Convert(DateTime,DateOUT,103) as [Date Out], RTRIM(NoOfDays) as [No. Of Days], RTRIM(NoOfMales) as [No. Of Males], RTRIM(NoOfFemales) as [No. Of Females], RTRIM(NoOfKids) as [No. Of Kids], RTRIM(NoOfExtraBed) as [No. Of Extra Bed], RTRIM(NoOfExtraPerson) as [No. Of Extra Person], RTRIM(RoomOrderCharges) as [Room Order Charges], RTRIM(ExtraPersonCharges) as [Extra Person Charges], RTRIM(TotalRoomCharges) as [Total Room Charges],RTRIM(ExtraBedCharges) as [Extra Bed Charges], RTRIM(OtherCharges) as [Laundry Charges], RTRIM(DiscountPer) as [Discount %], RTRIM(DiscountAmount) as [Discount Amount], RTRIM(STPer) as [ST %], RTRIM(STAmount) as [ST Amount], RTRIM(LuxuryTaxPer) as [Luxury Tax Per],RTRIM( LuxuryTaxAmount) as [Luxury Tax Amount], RTRIM(SubTotal) as [Sub Total], RTRIM(GrandTotal) as [Grand Total], RTRIM(TotalPaid) as [Total Paid], RTRIM(Balance) as [Balance], RTRIM(TaxPaidRate) as [Tax Paid Rate], RTRIM(Checkout_Room.Notes) as [Notes] from CheckOut_Room,Guest,Room,CheckIN_Room where Guest.ID=CheckIN_Room.GuestID and Checkout_Room.CheckInID=CheckIn_Room.Cin_ID and Room.RoomNo=CheckIn_Room.RoomNo and DateOut between @d1 and @d2 and Room.RoomNo=@d3 Order by DateOut", con)
            cmd.Parameters.Add("@d1", SqlDbType.DateTime, 30, "DateOut").Value = DateTimePicker2.Value.Date
            cmd.Parameters.Add("@d2", SqlDbType.DateTime, 30, "DateOut").Value = DateTimePicker1.Value
            cmd.Parameters.Add("@d3", SqlDbType.NChar, 50, "RoomNo").Value = cmbRoomNo.Text
            Dim myDA As SqlDataAdapter = New SqlDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "Guest")
            myDA.Fill(myDataSet, "Room")
    ...
    Private Sub Button4_Click(sender As System.Object, e As System.EventArgs) Handles Button4.Click
        Try
            ...
            Cursor = Cursors.WaitCursor
            Timer1.Enabled = True
            Dim rpt As New rptCheckOut1 'The report you created.
            Dim myConnection As SqlConnection
            Dim MyCommand, MyCommand1 As New SqlCommand()
            Dim myDA, myDA1 As New SqlDataAdapter()
            Dim myDS As New DataSet 'The DataSet you created.
            ...
            MyCommand.Connection = myConnection
            MyCommand1.Connection = myConnection
            MyCommand.CommandText = "SELECT Checkin_Room.Cin_Id,Guest.ID,CheckIN_Room.GuestID, CheckIN_Room.RoomNo, CheckIN_Room.RoomCharges, CheckIN_Room.DateIN, CheckIN_Room.DateOUT, CheckIN_Room.NoOfDays, CheckIN_Room.NoOfMales,CheckIN_Room.NoOfFemales, CheckIN_Room.NoOfKids, CheckIN_Room.NoOfExtraBed, CheckIN_Room.NoOfExtraPerson, CheckIN_Room.RoomOrderCharges, CheckIN_Room.ExtraPersonCharges, CheckIN_Room.TotalRoomCharges, CheckIN_Room.ExtraBedCharges, CheckIN_Room.OtherCharges, CheckIN_Room.DiscountPer, CheckIN_Room.DiscountAmount, CheckIN_Room.STPer,CheckIN_Room.STAmount, CheckIN_Room.LuxuryTaxPer, CheckIN_Room.LuxuryTaxAmount, CheckIN_Room.SubTotal, CheckIN_Room.GrandTotal, CheckIN_Room.TotalPaid, CheckIN_Room.Balance,CheckIN_Room.Status, CheckIN_Room.TaxPaidRate, CheckIN_Room.Notes, Room.RoomType, Guest.GuestName, Guest.Address, Guest.City, Guest.ContactNo, Guest.Gender, Guest.Occupation, Guest.Religion, Guest.IDType, Guest.IDNumber, Guest.Pic1, Guest.Pic2, Guest.pic3, Guest.Notes AS Expr5,Guest_Docs.Id AS Expr6, Guest_Docs.GuestID AS Expr7, Guest_Docs.Image1, Guest_Docs.Image2, Guest_Docs.Image3 FROM CheckIN_Room INNER JOIN Room ON CheckIN_Room.RoomNo = Room.RoomNo INNER JOIN Guest ON CheckIN_Room.GuestID = Guest.ID INNER JOIN Guest_Docs ON Guest.ID = Guest_Docs.GuestID where DateOut between @d1 and @d2 and Status='Checked Out' and Room.RoomNo=@d3 order by DateOut"
            ...
            MyCommand.Parameters.Add("@d3", SqlDbType.NChar, 50, "RoomNo").Value = cmbRoomNo.Text
            MyCommand1.CommandText = "SELECT * from Hotel"
            MyCommand.CommandType = CommandType.Text
            MyCommand1.CommandType = CommandType.Text
            myDA.SelectCommand = MyCommand
