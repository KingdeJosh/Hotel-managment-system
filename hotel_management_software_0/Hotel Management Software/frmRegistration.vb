...
Public Class frmRegistration
    ...
    Private Sub DeleteRecord()
        ...
        Try
            ...
            con.Open()
            Dim cl As String = "select UserID from Room_OrderInfo,Registration where Room_OrderInfo.Operator=Registration.UserID and UserID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl1 As String = "select UserID from Restaurant_BillingInfoKOT,Registration where Restaurant_BillingInfoKOT.Operator=Registration.UserID and UserID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl2 As String = "select UserID from Restaurant_OrderInfo,Registration where Restaurant_OrderInfo.Operator=Registration.UserID and UserID=@d1"
            cmd = New SqlCommand(cl2)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtUserID.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read Then
                MessageBox.Show("Unable to delete..Already in use in Restaurant Billing", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                If Not rdr Is Nothing Then
                    rdr.Close()
                End If
                Exit Sub
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl3 As String = "select UserID from Restaurant_OrderInfoKOT,Registration where Restaurant_OrderInfoKOT.Operator=Registration.UserID and UserID=@d1"
    ...
    Private Sub btnUpdate_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "update registration set userid=@d1, usertype=@d2,name=@d4,contactno=@d5,emailid=@d6,Active=@d8 where userid=@d7"
