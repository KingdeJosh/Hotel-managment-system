...
Public Class frmHotel
    Sub Reset()
        ...
        txtCIN.Text = ""
        txtAddress.Text = ""
        PictureBox1.Image = Hotel_Management_System.My.Resources.Resources.images__5__1
        txtHotelName.Focus()
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If txtHotelName.Text = "" Then
            MessageBox.Show("Please enter hotel name", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtHotelName.Focus()
            Return
        End If
        ...
        Try
            ...
            If rdr.Read Then
                MessageBox.Show("Record Already Exists" & vbCrLf & "please update the hotel info", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into Hotel( HotelName, Address, ContactNo, EmailID, TIN, STNo, CIN, Logo) VALUES (@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8)"
            ...
            cmd.ExecuteNonQuery()
            con.Close()
            Dim st As String = "added the hotel '" & txtHotelName.Text & "' info"
            LogFunc(lblUser.Text, st)
            MessageBox.Show("Successfully saved", "Hotel Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update Hotel set HotelName=@d1, Address=@d2, ContactNo=@d3, EmailID=@d4, TIN=@d5, STNo=@d6, CIN=@d7, Logo=@d8 where ID=" & txtID.Text & ""
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtHotelName.Text)
            cmd.Parameters.AddWithValue("@d2", txtAddress.Text)
            ...
            cmd.ExecuteNonQuery()
            con.Close()
            Dim st As String = "updated the hotel '" & txtHotelName.Text & "' info"
            LogFunc(lblUser.Text, st)
            MessageBox.Show("Successfully updated", "Hotel Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
            btnUpdate.Enabled = False
            Getdata()
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.[Error])
    ...
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(ID), RTRIM(HotelName), RTRIM(Address), RTRIM(ContactNo), RTRIM(EmailID), RTRIM(TIN), RTRIM(STNo), RTRIM(CIN), Logo from Hotel", con)
    ...
    Private Sub DeleteRecord()
        Try
            ...
            If RowsAffected > 0 Then
                Dim st As String = "deleted the hotel '" & txtHotelName.Text & "' info"
                LogFunc(lblUser.Text, st)
