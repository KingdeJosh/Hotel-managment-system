...
Imports System.Text
Imports System.IO
Public Class frmDeliveryPerson
    ...
    Private Sub auto()
        ...
        con.Open()
        Dim sql As String = ("SELECT MAX(EmpID) FROM DeliveryPerson")
        cmd = New SqlCommand(sql)
        cmd.Connection = con
        If (IsDBNull(cmd.ExecuteScalar)) Then
            Num = 1
            txtID.Text = Num.ToString
            txtEmployeeID.Text = "D-" + Num.ToString
        Else
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into DeliveryPerson(Empid,employeeid,employeename,address,City,contactno,email,dateofjoining,photo,Active) VALUES (" & txtID.Text & ",@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8,'" & st1 & "')"
    ...
    Public Sub DeleteRecord()
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl As String = "select DeliveryPerson.EmpID from DeliveryPerson,RestaurantPOS_BillingInfoHD where DeliveryPerson.EmpID=RestaurantPOS_BillingInfoHD.Employee_ID and DeliveryPerson.EmpID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cq As String = "delete from DeliveryPerson where EmpID=" & txtID.Text & ""
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "update DeliveryPerson set employeeid=@d1,employeename=@d2,address=@d4,City=@d5,contactno=@d6,email=@d7,dateofjoining=@d11,photo=@d14,Active='" & st1 & "' where Empid=" & txtID.Text & ""
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtEmployeeID.Text)
            cmd.Parameters.AddWithValue("@d2", txtEmployeeName.Text)
